...
export const MoviePage = () => {
  ...
  const { movie, genreIds, productionCountries } = useMovieDetails(
    movieId.pathname.toString()
  );
  ...
  return (
    <Container>
      ...
      <Section
        actors
        title={
          cast.length > 0 ? `Cast (${cast.length})` : "No cast available 😥"
        }
        body={
          cast.length > 0
            ? cast.map((cast) => (
      ...
      <Section
        actors
        title={
          crew.length > 0 ? `Crew (${crew.length})` : "No crew available 😥"
        }
        body={
          crew.length > 0
            ? crew.map((member) => (
